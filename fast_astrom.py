...
def find_nearest_mod(number, mod_number, mode='up'):
    ...
    to PCAT's multiple-region MCMC approach (part of pcat-lion, or "Lion").
...
class wcs_astrometry():
    """
    This class contains the WCS header and other information necessary to construct arrays for fast 
    astrometric transformations. The fast astrometry assumes linear interpolation is sufficiently accurate for 
    transforming coordinates across position and across different bands. 
    ...
    def change_verbosity(self, verbtype):
        """ Function intended to change verbosity if desired. Have not really tested this yet """
        self.verbosity = verbtype

    def change_base_path(self, basepath):
        """ Function intended to change base file path if desired. Not fully specified within the code so wouldn't use it right now."""
    ...
    def get_pint_dp(self, p):
        """ Grab the integer and non-integer parts of a float (or array of floats).
        ...
        Returns
        -------
        pint : integer component of p
        dp : non-integer component of p
        """
        pint = np.floor(p+0.5)
        dp = p - pint
    ...
    def get_derivative(self, idx0, idx1, x, y, epsx, epsy):
        """
        Compute numerical derivatives from first differences. 
        """
