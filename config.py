''' This configuration file contains the main parameters for PCAT-DE. These are the defaults unless otherwise specified when instantiating PCAT. '''

ROOT = '/Users/richardfeder/'
...
relative_dust_sed_dict = dict({'S':sed_cirrus[250]/sed_cirrus[100],\
							   'M':sed_cirrus[350]/sed_cirrus[100],\
							   'L':sed_cirrus[500]/sed_cirrus[100]}) # relative to 100 micron

temp_mock_amps_dict = dict({'S':0.03, 'M': 0.2, 'L': 0.8}) # MJy/sr, this was for RXJ1347
spire_band_names = ['PSW', 'PMW',  'PLW']
spire_band_dict = dict({0:'S',1:'M',2:'L'})
spire_band_names_dict = dict({0:'PSW',1:'PMW',2:'PLW'})
lam_dict = dict({'S':250, 'M':350, 'L':500})
pixsize_dict = dict({'S':6., 'M':8., 'L':12.}) # arcseconds
spire_bands = ['S', 'M', 'L']
nbl = np.arange(len(spire_bands))
fourier_band_idxs = nbl.copy()
template_bands_dict = dict({'sze':['M', 'L'], 'szetemp':spire_bands, 'lensing':spire_bands, \
								'dust':spire_bands, 'planck':spire_bands}) # should just integrate with the same thing in Lion main
template_band_idxs_dict = dict({'sze':nbl, 'sze':nbl, 'lensing':nbl, 'dust':nbl, 'planck':nbl, 'cib':nbl})
temp_amplitude_sigs = dict({'sze':0.001, 'dust':0.1, 'planck':0.05, 'fc':0.001, 'binned_cib':0.0005}) # binned cib
...
# I think we should get rid of width/height/x0/y0 within the implementation.
